...
world_map_button = tk.Button(canvas, text='Show World Map', bg='#d7dbd7', font=('Calibri', 15),
                             command=lambda: check_input(worldmap=True))
world_map_button.place(relx=0.75, rely=0.4, relwidth=0.18, relheight=0.05)

# Make a button to search for new COVID-19 cases
update_button = tk.Button(canvas, text='Get COVID-19 Updates', bg='#d7dbd7', font=('Calibri', 15),
                          command=lambda: get_updates())
update_button.place(relx=0.75, rely=0.5, relwidth=0.18, relheight=0.05)

# Make a label for new COVID-19 cases
new_cases_label = tk.Label(canvas, text='', font=('Calibri', 12))
new_cases_label.place(relx=0.75, rely=0.6, relwidth=0.18, relheight=0.1)
...
def get_data():
    ...
    past_data = requests.get(url)
    past_data = past_data.json()
    corona_cases = past_data['todayCases']
    active_cases = past_data['active']
    total_cases = past_data['cases']
    return corona_cases, active_cases, total_cases


# Global variable for COVID-19 updates
corona_cases_1 = get_data()[0]
date_filename = 'date.json'
cases_filename = 'cases.json'

try:
    with open(date_filename) as f_obj:
        current_time = json.load(f_obj)
        first_time = False
except FileNotFoundError:
    current_time = datetime.now().strftime('%m/%d/%Y %I:%M %p')
    with open(date_filename, 'w') as f_obj:
        json.dump(current_time, f_obj)
    with open(cases_filename, 'w') as f_obj:
        json.dump(corona_cases_1, f_obj)
    first_time = True


def get_updates():
    corona_cases_2 = get_data()[0]
    active_cases_2 = get_data()[1]
    global corona_cases_1
    global current_time
    global first_time
    if first_time:
        new_cases_label['text'] = f'{active_cases_2} currently active cases \nof COVID-19.'
        corona_cases_1 = get_data()[0]
        with open(cases_filename, 'w') as f_obj:
            json.dump(corona_cases_1, f_obj)
        first_time = False
    elif not first_time:
        with open(cases_filename) as f_obj:
            corona_cases_1 = json.load(f_obj)
        if corona_cases_2 > corona_cases_1 != corona_cases_2:
            cases_diff = corona_cases_2 - corona_cases_1
            new_cases_label[
                'text'] = f'{cases_diff} more cases of \nCOVID-19 since {current_time}. \n{active_cases_2} total ' \
                          f'active cases currently. '
            corona_cases_1 = corona_cases_2
            current_time = datetime.now().strftime('%m/%d/%Y %I:%M %p')
            with open(date_filename, 'w') as f_obj:
                json.dump(current_time, f_obj)
            with open(cases_filename, 'w') as f_obj:
                json.dump(corona_cases_1, f_obj)
        elif corona_cases_1 > corona_cases_2:
            cases_diff = corona_cases_2
            new_cases_label[
                'text'] = f'At least {cases_diff} more cases of \nCOVID-19 since yesterday.\n{active_cases_2} total ' \
                          f'active cases currently. '
            corona_cases_1 = corona_cases_2
            current_time = datetime.now().strftime('%m/%d/%Y %I:%M %p')
            with open(date_filename, 'w') as f_obj:
            ...
            new_cases_label['text'] = f'No new cases of COVID-19 \nsince {current_time}'
...
search_button = tk.Button(canvas, text='Create Graph', bg='#d7dbd7', font=('Calibri', 15),
...
def check_input(worldmap=None):
    ...
    if len(list(filtered_list)) == len(entry_list):  # Make sure there's input for every entry.
        for country in entry_list:
            if country.entry.get().strip().lower() != 'uk' and country.entry.get().strip().lower() != 'usa':
                ...
                try:
                    ...
                    return
        try:  # Make sure dates correctly formatted
            start = datetime.strptime(start_date_entry.get().lstrip('0'), '%m/%d/%y')
            end = datetime.strptime(end_date_entry.get().lstrip('0'), '%m/%d/%y')
        except ValueError:
            error_message(2)  # Date formatting error
        else:
            if start < end < datetime.today() and worldmap:
                show_world_map()  # If all these tests for user input pass, proceed to search data.
            elif start < end < datetime.today():
                search_entries()
            else:
                error_message(2)  # Date formatting error
    else:
...
def add_province_data(country_list, country_name):
    ...
    for country in country_list:  # Parse through the data to add up the data from the provinces of one country
        if country['country'].lower().strip() == country_name.lower().strip():
            cases = [sum(i) for i in zip(cases, list(country['cases'].values()))]
...
def search_data(entry_num):
    ...
    if country_name.title().strip() == 'United States':
        ...
        country_name = 'uk'
    elif country_name.lower().strip() != 'uk' and country_name.lower().strip() != 'usa':
        # Exceptions for countries spell check.
...
def plot_data(data):
    ...
    for i in range(len(entry_list)):  # Loop through the data on each country and plot it.
        if entry_list[i].entry.get().lower().strip() == 'uk':
            plt.plot(dates, data[i], label='UK')
        elif entry_list[i].entry.get().lower().strip() == 'usa':
...
def show_graph():
    ...
    # Label for displaying graph image
    img_label = tk.Label(graph_frame, font=('Calibri', 30))
    ...
    for database_country in database_countries:
        for country in request_response:
            if database_country == country['country']:
                case = country['timeline']['cases'][end_date_entry.get().strip()]
                try:
...
def filter_countries(world_map_dict, cases_dict, world_countries):
    ...
    for country in cases_dict.keys():
        for world_country in world_countries:
            if country.lower().strip() == world_country.lower().strip():
...
def show_world_map():
    ...
    world_countries = [item for item in pygal_maps_world.maps.COUNTRIES.values()]
    world_codes = [item for item in pygal_maps_world.maps.COUNTRIES.keys()]
    ...
    url = os.path.realpath('covid_19_world_map.svg')
...
def error_message(error_type):
    ...
    if error_type == 1:
        ...
        error_label = tk.Label(graph_frame, text='Make sure the dates are formatted correctly and the end date is not '
                                                 'today! \n Formatting example: 1/1/20', font=('Calibri', 17))
        error_label.place(relx=0, rely=0, relwidth=1, relheight=1)
    elif error_type == 3:
